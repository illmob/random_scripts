...
banner() {
    ...
    echo -e "\e[34m    Usage:\e[0m certs.sh [domain.name]\n"
}
banner  # show the banner

# check if a domain name was entered
if [ -z "$*" ]; then echo -e "[\033[31mx\e[0m]\033[31m ERROR:\e[0m Need a domain name" >&2; exit 1;fi
...
echo -e "├── [\e[92m*\e[0m] GETTING SUBDOMAINS"
...
echo -e $crtsh $certspotter $hackertarget | tr -s " " "\n" | tr '[:upper:]' '[:lower:]' |sort -u -o subdomains.txt
